...
struct DijetPair {
  ...
  double
      closeness; // = M_PI - dphi (smaller is better / closer to back-to-back)
};
...
int countInCone(const PartContainer &parts, double eta0, double phi0, double R,
                double partPtMin, double partEtaMax) {
...
int main(int argc, char *argv[]) {
  ...
  const std::string labMax =
      (ptHatMax > 0.0) ? trim_trailing_zeros(ptHatMax) : "-1";
  ...
  double lead_pt, sub_pt, lead_eta, sub_eta, lead_phi, sub_phi, closeness,
      background_mult_A, background_mult_B;
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
    ...
    std::sort(myPairs.begin(), myPairs.end(),
              [&](const DijetPair &A, const DijetPair &B) {
    ...
    for (const auto &pair : chosenPairs) {
      ...
      background_mult_A =
          countInCone(parts, lead_eta, phiB, jetRadius, partPtMin, partEtaMax);
      background_mult_B =
          countInCone(parts, lead_eta, phiA, jetRadius, partPtMin, partEtaMax);
  ...
  vector<TString> statNames = {"nEvents",  "nAccepted",   "ptHatMin",
                               "ptHatMax", "sigmaGen_mb", "sigmaErr_mb"};
  ...
  std::cout << "Accepted dijet-like events: " << accepted << " / " << nEvents
            << std::endl;
  return 0;
}
